import ConnectedPlanning from "./ConnectedPlanning";
...
export default function Dashboard({ storageId = 'demo', user = null, onAccount }) {
  ...
  const model = isSample ? sample : { entries: [] };
  ...
  const freshEventId=useRef(crypto.randomUUID());
  ...
  const out=monthly.filter(e=>e.type!=='income').reduce((a,e)=>a+Math.round(e.amount*100),0)/100;
  ...
  function add(category=isSample?'Home & bills':workspace.categories?.find(item=>!item.archived)?.name||categories[0]) {freshEventId.current=crypto.randomUUID();opener.current=document.activeElement;setEditorError('');setForm({id:freshEventId.current,isNew:true,name:'',amount:'',date:month===today.slice(0,7)?today:`${month}-01`,type:'expense',category,categoryId:workspace.categories?.find(item=>item.name===category)?.id});}
  async function remove(entry) {setBusy(true);try{const result=await workspace.deleteEvent(entry.id);setRemoved({...entry,revision:result?.revision||entry.revision});setForm(null);}catch(error){setEditorError(error.message||'Could not remove this event. Please try again.');}finally{setBusy(false);}}
  async function save(event) {event.preventDefault();const amount=Number(form.amount);if(!form.name.trim()||!Number.isFinite(amount)||amount<=0)return;const entry={...form,name:form.name.trim(),amount:Math.round(amount*100)/100,id:form.id||freshEventId.current};delete entry.isNew;if(!entry.recurring?.frequency)delete entry.recurring;setBusy(true);try{await workspace.saveEvent(entry);setMonth(entry.date.slice(0,7));setForm(null);}catch(error){setEditorError(error.message||'Could not save changes. Please try again.');}finally{setBusy(false);}}
  if(workspace.loading)return <section className="dashboard-page" aria-live="polite"><p>Loading your workspace…</p></section>;
  return <WorkspaceShell sampleReset={isSample&&tab!=='design-review'?()=>{sample.reset();planningDispatch({type:'resetLimits'});planningDispatch({type:'resetScenario'});setFlowState({goal:null,monthly:{},subscription:null,paused:[],filter:'All'});setAnalysis({days:30,date:0,month:'2026-08',category:null});setMonth('2026-09');setQuery('');setKind('all');setStatus('all');setRemoved(null);}:undefined} page={tab} scope={scope} identity={storageId==='demo'?'Sample workspace':workspace.isCloud?'Synced workspace':'Local workspace'} onAccount={onAccount} signedIn={Boolean(user)} month={month} onMonth={setMonth} busy={busy} action={['schedule','overview'].includes(tab)?<button className="ledger-add" disabled={!isSample&&!workspace.capabilities?.write} onClick={()=>add()}>+ Add event</button>:null}>

    {!isSample&&!workspace.loading&&!workspace.capabilities?.write&&<p className="status-message" role="status">This workspace is read-only while the new backend is being prepared. Your records are preserved.</p>}
    {!isSample&&(!workspace.isCloud||import.meta.env.VITE_WORKSPACE_V2_ENABLED==='true')&&workspace.capabilities?.schemaVersion!==2&&<section className="migration-panel"><h2>Enable the new planning workspace.</h2><p>{workspace.isCloud?'Enable an empty cloud workspace for connected planning. Existing records require a verified migration.':'Keep a local backup and upgrade your records to use connected budgets, goals, and forecasts.'}</p><button className="ledger-add" disabled={busy} onClick={async()=>{setBusy(true);try{await workspace.upgrade(today);}catch{/* displayed by workspace */}finally{setBusy(false);}}}>Back up and upgrade</button></section>}
    {workspace.error&&<p className="status-message status-error" role="alert">{workspace.error}{workspace.isCloud&&<button className="text-action" onClick={workspace.retry}>Retry sync</button>}</p>}
    {workspace.migration&&<section className="migration-panel" aria-labelledby="migration-title"><h2 id="migration-title">Bring this device’s plan with you?</h2><p>{workspace.migration.modern?`We found a verified local backup with ${workspace.migration.backup.manifest.recordCount} records.`:`We found ${workspace.migration.entries.length} validated personal events and ${workspace.migration.goals.length} goals.`} Nothing from the sample demo will be copied.</p><div><button className="ledger-add" disabled={busy} onClick={async()=>{setBusy(true);try{await workspace.migratePersonal();}finally{setBusy(false);}}}>Migrate my personal plan</button><button className="text-action" onClick={workspace.dismissMigration}>Not now</button></div></section>}
    ...
    {tab==='design-review'?<DesignReview/>:tab==='overview'?<WorkspaceOverview monthly={monthly} goals={workspace.goals} income={income} out={out} available={available} mood={mood} face={<Smiley mood={mood}/>} today={today} onEdit={openEditor} onNavigate={setTab}/>:['goals','roadmap'].includes(tab)?<WorkspaceGoals today={today} workspace={workspace} page={tab} state={flowState} setState={setFlowState} onNavigate={setTab}/>:planningIds.includes(tab)&&!isSample&&workspace.capabilities?.planning?<ConnectedPlanning key={tab} page={tab} workspace={workspace} period={month} today={today}/>:planningIds.includes(tab)&&!isSample?<section className="schedule-empty"><h2>Your workspace, your records.</h2><p>This page is awaiting its live data connection. Your schedule and goals remain available.</p><button className="ledger-add" onClick={()=>setTab('schedule')}>Open your schedule</button><button className="text-action" onClick={()=>setTab('design-review')}>Explore the design preview</button></section>:planningIds.includes(tab)?<PlanningStudio key={tab} page={tab} model={planningModel} analysis={analysis} setAnalysis={setAnalysis} flowState={flowState} setFlowState={setFlowState} planningState={planningState} planningDispatch={planningDispatch} onNavigate={setTab}/>:tab==='allocation'?<Insights entries={entries} month={month} onSchedule={()=>setTab('schedule')}/>:['more','profile','settings','help'].includes(tab)?<MorePages key={tab} page={tab} onNavigate={setTab} workspace={workspace}/>:<WorkspaceSchedule entries={scheduled} today={today} query={query} onQuery={setQuery} kind={kind} onKind={setKind} status={status} onStatus={setStatus} onEdit={openEditor} totalCount={monthly.length}/>}

    {form&&<dialog onKeyDown={containDialogFocus} className="entry-dialog event-editor" aria-label={form.id&&!form.isNew ? "Edit event" : "Add event"} ref={dialog} onCancel={e=>{e.preventDefault();closeEditor();}}><form onSubmit={save}><div className="event-editor-body"><div className="entry-heading"><h2>{form.id&&!form.isNew?'Edit event':'New event'}</h2><button type="button" aria-label="Close editor" onClick={closeEditor}>×</button></div><p className="drawer-description">Keep the details clear. Your monthly plan updates when you save.</p>{editorError&&<div className="drawer-error"><p role="alert">{editorError}</p>{!form.isNew&&<button type="button" disabled={busy} onClick={()=>{const saved=workspace.entries.find(entry=>entry.id===form.id);if(saved){originalForm.current=JSON.stringify(saved);setForm({...saved});setEditorError('');}}}>Reload saved event</button>}</div>}<div className="entry-type">{['expense','income',...(workspace.capabilities?.schemaVersion===2?['saving']:[])].map(type=><button type="button" key={type} aria-pressed={form.type===type} onClick={()=>setForm({...form,type,goalId:type==='saving'?form.goalId:null,category:form.category||categories[0]})}>{type==='income'?'+ Money in':type==='saving'?'Savings':'− Money out'}</button>)}</div><label>Name<input autoFocus required maxLength={100} value={form.name} placeholder={form.type==='income'?'e.g. Freelance payment':'e.g. Internet bill'} onChange={e=>setForm({...form,name:e.target.value})}/></label><div className="entry-fields"><label>Amount (USD)<input required type="number" inputMode="decimal" min="0.01" max="1000000" step="0.01" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})}/></label><label>Date<input required type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})}/></label></div>{<label>Category<select value={form.category||categories[0]} onChange={e=>setForm({...form,category:e.target.value,categoryId:workspace.categories?.find(item=>item.name===e.target.value)?.id})}>{(isSample?[...sampleCategories.map(item=>item.name),'Savings']:workspace.categories?.filter(item=>!item.archived).map(item=>item.name)||categories).map(c=><option key={c}>{c}</option>)}</select></label>}{form.type==='saving'&&<label>Goal (optional)<select value={form.goalId||''} onChange={e=>setForm({...form,goalId:e.target.value||null})}><option value="">Unallocated savings</option>{workspace.goals.filter(goal=>!goal.archived).map(goal=><option key={goal.id} value={goal.id}>{goal.name}</option>)}</select></label>}<label>Repeat<select value={form.recurring?.frequency||''} onChange={e=>setForm({...form,recurring:e.target.value?{frequency:e.target.value,interval:1}:null})}><option value="">Does not repeat</option><option value="daily">Every day</option><option value="weekly">Every week</option><option value="monthly">Every month</option><option value="yearly">Every year</option></select></label>{form.recurring?.frequency&&<div className="entry-fields"><label>Repeat every<input type="number" min="1" max="365" step="1" value={form.recurring.interval||1} onChange={e=>setForm({...form,recurring:{...form.recurring,interval:Number(e.target.value)}})}/></label><label>End date (optional)<input type="date" min={form.date} value={form.recurring.endDate||''} onChange={e=>setForm({...form,recurring:{...form.recurring,endDate:e.target.value||undefined}})}/></label></div>}{<label>Status<select value={form.status||'planned'} onChange={e=>setForm({...form,status:e.target.value})}><option value="planned">Planned</option><option value="completed">{form.type==='income'?'Received':form.type==='saving'?'Saved':'Paid'}</option></select></label>}{form.recurring?.frequency&&<p className="ledger-footnote">{workspace.isCloud?"Future events repeat at this amount until the end date, if set.":"Sign in to generate future events automatically."}</p>}</div><div className="event-editor-actions"><button disabled={busy||(!isSample&&!workspace.capabilities?.write)} className="ledger-save" type="submit">{busy?'Saving…':form.id&&!form.isNew?'Save changes':'Add event'}</button><div className="drawer-secondary"><button type="button" disabled={busy} onClick={closeEditor}>Cancel</button>{form.id&&!form.isNew&&<button disabled={busy||(!isSample&&!workspace.capabilities?.write)} className="ledger-delete" type="button" onClick={()=>remove(form)}>Remove event</button>}</div></div></form></dialog>}
  </WorkspaceShell>;
}
